...
CREATE TABLE IF NOT EXISTS `store_db`.`orderline` (
  ...
  CONSTRAINT `order_id` FOREIGN KEY (`order_id`) REFERENCES `store_db`.`orders` (`order_id`),
  CONSTRAINT `product_id` FOREIGN KEY (`product_id`) REFERENCES `store_db`.`products` (`product_id`)
);
